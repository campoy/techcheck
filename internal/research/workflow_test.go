...
// The M2 linear workflow: plan -> search -> extract -> brief -> write, in
// that order, with the default search budget applied when none is given.
func TestCompanyResearchLinear(t *testing.T) {
	...
	env.OnActivity(a.GenerateBrief, mock.Anything, research.BriefRequest{Company: "acme", Findings: findings}).
	...
	require.Equal(t, []string{"plan", "search", "extract", "brief", "write"}, order,
		"linear M2 pipeline must run in order")
